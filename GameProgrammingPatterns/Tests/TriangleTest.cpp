...
void line(Vec2i p0, Vec2i p1, TGA_Image &image, TGA_Color color) {
	bool steep = false;
...
int main(int argc, char** argv) {
	...
	for (int i = 0; i<model->nfaces(); i++) {
		...
		if (intensity > 0) {
			triangle(screen_coords[0], screen_coords[1], screen_coords[2], image, TGA_Color(intensity * 255, intensity * 255, intensity * 255, 255));
		}
	}
