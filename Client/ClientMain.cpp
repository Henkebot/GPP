...
global OPTIONS default_options = { "localhost","27015", 1, BUFFER_SIZE, TRUE };
...
global DWORD WINAPI EchoThread(LPVOID lpParameter)
{
	...
	sprintf_s(outbuf, BUFFER_SIZE, "Henrik Was Here!");
	std::string buffer;
	...
	if ((inbuf) && (outbuf))
	{
		...
		while (TRUE)
		{
			...
			sprintf_s(outbuf, BUFFER_SIZE, buffer.c_str());
...
global BOOL SendBuffer(int nThreadNum, char * outbuf)
{
	...
	int nSend = 0;
	if (outbuf[0] == 'l')
		closesocket(g_ThreadInfo.sd[nThreadNum]);
